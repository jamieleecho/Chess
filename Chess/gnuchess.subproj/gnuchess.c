...
/* glue functions */
extern void ShowMessage(), UpdateDisplay(), ElapsedTime(), ClrScreen();
extern void SetTimeControl(), SelectLevel(), GetOpenings(), ShowDepth();
extern void ShowResults(), ShowSidetomove(), SearchStartStuff(), OutputMove();
extern void GameEnd(), ShowCurrentMove();

/* functions for glue */
void  gnuchess_main_init(), NewGame();
extern void SelectMoveStart( struct MoveInfo *mi );
void  InitializeStats(), SelectLoop(), algbr();
int   VerifyMove(), SelectMoveEnd(), SqAtakd(), castle(), distance();

/* internal functions */
static void  UnmakeMove(), MoveList(), MakeMove(), ZeroTTable(), pick();
static int   search(), ProbeTTable();
static void  ExaminePosition(), ScorePosition(), OpeningBook(), repetition();
static short evaluate(), SqValue();
static void  CaptureList(), ataks(), ScoreLoneKing();
void PutInTTable(short side, short score, short depth, short alpha, short beta, short mv);
static void  GenMoves(), LinkMove(), UpdateHashbd(), UpdatePieceList();
static int   ScoreKPK(), ScoreKBNK(), trapped();
static void  UpdateWeights(), BRscan(), KingScan(), CopyBoard(), BlendBoard();
...
short quit,reverse,bothsides,hashflag,InChk,player,force,easy,chess_beep;
short wking,bking,FROMsquare,TOsquare,chess_timeout,Zscore,zwndw,xwndw,slk;
short INCscore;
...
*/
{
  ...
  while (!chess_timeout && Sdepth < MaxSearchDepth)
    SelectLoop( &mi );
...
struct leaf *node,tmp;
  ...
  for (pnt = pbst = TrPnt[ply]; pnt < TrPnt[ply+1] && best <= beta; pnt++){
    ...
    if (node->score > best && (!chess_timeout || Sdepth < 2)){
      if (depth > 0)
...
short evaluate(side,xside,ply,alpha,beta)
